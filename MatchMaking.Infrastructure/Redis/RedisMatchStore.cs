    ...
    ILogger<RedisMatchStore> logger) : IMatchStore
{
    ...
    public async Task SaveMatchAsync(Match match, TimeSpan ttl, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<Match?> GetMatchByMatchIdAsync(string matchId, CancellationToken cancellationToken = default)
    {
        var key = RedisKeyNames.Match(matchId);
        var json = await _db.StringGetAsync(key);
        if (json.IsNullOrEmpty) return null;
        var dto = JsonSerializer.Deserialize<MatchDto>(json!);
        return dto is null ? null : new Match(dto.MatchId, dto.UserIds, dto.CreatedAtUtc);
    }

    public async Task SetUserMatchAsync(string userId, string matchId, TimeSpan ttl, CancellationToken cancellationToken = default)
    {
        var key = RedisKeyNames.UserMatch(userId);
        await _db.StringSetAsync(key, matchId, ttl);
    }

    public async Task<Match?> GetMatchByUserIdAsync(string userId, CancellationToken cancellationToken = default)
    {
